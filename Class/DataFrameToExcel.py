...
class DataFrameToExcel:
    def __init__(self, df, initial_file_name=None, master=None, show_gui=True):
        ...
        if master is None:
            self.root = tk.Tk()
            self.root.title("DataFrame to Excel")
            self.addloop = True
        else:
            self.root = tk.Toplevel(master)
            self.root.title("DataFrame to Excel Settings")
            self.root.geometry("600x450+1100+300")
            self.addloop = False
    ...
    def setup_gui(self):
        ...
        Label(self.root, text="Title font color:").grid(
            row=3, column=0, padx=20, pady=5, sticky=E
        )
        Entry(self.root, textvariable=self.title_font_color, width=20).grid(
        ...
        Label(self.root, text="File name:").grid(
            row=4, column=0, padx=20, pady=5, sticky=E
        )
        Entry(self.root, textvariable=self.file_name, width=20).grid(
            row=4, column=1, padx=20, pady=5
        )

        Label(self.root, text="Correct date format:").grid(
        ...
        Label(self.root, text="Highlight rows:").grid(
            row=6, column=0, padx=20, pady=5, sticky=E
        )
        Radiobutton(
            self.root, text="Yes", variable=self.highlight_rows, value="yes"
        ).grid(row=6, column=1, padx=20, pady=5, sticky=W)
        Radiobutton(
            self.root, text="No", variable=self.highlight_rows, value="no"
        ).grid(row=6, column=1)

        Label(self.root, text="Highlight column:").grid(
            row=7, column=0, padx=20, pady=5, sticky=E
        )
        self.highlight_column_menu = OptionMenu(
            self.root, self.highlight_column, *self.df.columns
        )
        ...
        Label(self.root, text="Highlight value:").grid(
            row=8, column=0, padx=20, pady=5, sticky=E
        )
        Entry(self.root, textvariable=self.highlight_value, width=20).grid(
            row=8, column=1, padx=20, pady=5
        )

        Label(self.root, text="Highlight color:").grid(
            row=9, column=0, padx=20, pady=5, sticky=E
        )
        Entry(self.root, textvariable=self.highlight_color, width=20).grid(
        ...
        self.date_columns_listbox = Listbox(
            self.root, selectmode=MULTIPLE, exportselection=0, width=20, height=4
        )
        for col in self.df.columns:
            self.date_columns_listbox.insert("end", col)

        if self.correct_date_format.get() == "yes":
            Label(self.root, text="Date columns:").grid(
        ...
        save_button = Button(
            self.root, text="Save to Excel", command=self.save_to_excel
        )
